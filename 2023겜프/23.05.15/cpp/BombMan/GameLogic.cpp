...
void Render(char _cMaze[VERTICAL][HORIZONTAL], PPLAYER _pPlayer, vector<POS>& _vecBoomEffect)
{
	for (int i = 0; i < VERTICAL; ++i)
	{
		for (int j = 0; j < HORIZONTAL; ++j)
		{
			...
			for (int k = 0; k < _vecBoomEffect.size(); ++k)
			{
				if (_vecBoomEffect[k].x == j && _vecBoomEffect[k].y == i)
				{
...
void Event(std::vector<BOOM>& _vecBomb, std::vector<POS>& _vecBoomEffect)
{
	...
	std::vector<POS>::iterator effectIter;

	for (effectIter = _vecBoomEffect.begin(); effectIter != _vecBoomEffect.end();)
	{
		effectIter = _vecBoomEffect.erase(effectIter);
	}
...
void Fire(char _cMaze[VERTICAL][HORIZONTAL], PPLAYER _pPlayer, POS _boomPos, std::vector<POS>& _vecBoomEffect)
{
	...
	for (int i = iScopeMinX; i <= iScopeMaxX; ++i)
		_vecBoomEffect.push_back({ i, _boomPos.y });
	for (int i = iScopeMinY; i <= iScopeMaxY; ++i)
		_vecBoomEffect.push_back({ _boomPos.x, i });

	for (auto& target : _vecBoomEffect)
	{
